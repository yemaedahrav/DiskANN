...
template <typename T, typename TagT = uint32_t, typename LabelT = uint32_t> class Index : public AbstractIndex
{
  ...
  protected:
    ...
    std::pair<uint32_t, uint32_t> iterate_to_fixed_point(InMemQueryScratch<T> *scratch, const uint32_t Lindex,
                                                         const std::vector<uint32_t> &init_ids, bool use_filter, std::vector<bool> &cluster_centre_status, const std::vector<LabelT> &filters, bool search_invocation);

    void search_for_point_and_prune(int location, uint32_t Lindex, std::vector<uint32_t> &pruned_list,
                                    InMemQueryScratch<T> *scratch, std::unordered_map<uint32_t, std::set<uint32_t>> &node_to_cluster, std::unordered_map<uint32_t, std::set<uint32_t>> &cluster_to_node, std::vector<bool> &cluster_centre_status, bool second_pass, bool use_filter = false,
    ...
    // Acquire exclusive _update_lock before calling
    void link(std::vector<bool> &cluster_centre_status, std::unordered_map<uint32_t, std::set<uint32_t>> &cluster_to_node, std::unordered_map<uint32_t, std::set<uint32_t>> &node_to_cluster);
